<template>
  <div id="comment">
    ...
    <div class="container">
      ...
      <div class="img-w">
        <ul class="clear">
          <li class="img-list" v-for="item in comment.imgList" :style="{backgroundImage: `url(${item.img})`}"><span
      ...
      <div class="history-w">
        <ul class="group">
          <li class="list" v-for="(item,key) in commentsData.list" :key="key">
            ...
            <div class="tag-b">
              <span class="tag-list tag_a" v-for="(item1,key1) in item.tagList">{{item1.text}}</span>
            </div>
            <div class="content-b">
              {{item.content}}
            </div>
            <ul class="img-b clear">
              <li class="img"
                  v-for="(item2,key2) in item.imgList"
                  :key="key2"
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      ...mapActions("lesson", ["getTagList", "getHistoryComment"]),
      chooseStar(n) {
...
//              });
      ...
      deleteImg(id) {
        Indicator.open({spinnerType: 'fading-circle'});
        console.log(id);
        this.deleteImgAjax({id: id}).then(res => {
          Indicator.close();
          if(res.status == "0") {
...
//      });
      //获取历史评价列表
//      this.getHistoryComment().then(res=>{
//        console.log(res.data);
